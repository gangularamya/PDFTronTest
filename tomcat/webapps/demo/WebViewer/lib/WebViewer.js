...
(function(root, PDFTron) {
  ...
  */
  ...
  PDFTron.WebViewer.prototype = {
    ...
    _createViewer: function(viewers) {
      ...
      for (var i = 0; i < viewers.length; i++) {
        if (viewers[i].toLowerCase() === 'html5mobile') {
          if (this.options.documentType && this.options.documentType !== 'xod') {
            continue;
          }
    ...
    loadDocument: function(url, options) {
      var existingLoadOptions = {
        ...
        pdftronServer: this.options.pdftronServer
      };
   ...
   * @property {string} [documentType] The type of document the viewer will be used with. Valid values are "xod", "pdf", "office" and "all". When loading images directly the "pdf" type should be used. (default is "xod").
   ...
   * @property {boolean} [pdfnet=false] A boolean to enable the use of PDFNet.js library functions (PDF only).
